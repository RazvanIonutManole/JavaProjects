...
public class Buffon extends Frame{
	...
	public Buffon(){
		...
		hh=res.height;
		setResizable(false);
		setTitle("Estimarea numarului PI folosind problema lui Buffon");
		...
		add(simPanel);
		simPanel.setBounds(25,100,450,450);
...
class SimPanel extends Panels{
	...
	void simulare(int nr){
		...
		double theta=0.0;
		for(int i=0;i<nr;i++){
			x=25.0+Math.random()*400.0;
			y=25.0+Math.random()*400.0;
			...
			for(int j=1;j<5;j++){
				if((points[i].x - 225)*(points[i].x - 225)+(points[i].y - 225)*(points[i].y-225)<=200*200){
					nrCazuriFavorabile++;
					break;
				}
			}
		...
		if(nrCazuriFavorabile!=0)
			pi=(4.0*(double)nrCazuriFavorabile/(double)nrAruncari);
		else pi=0.0;
		repaint();
	}
	...
	public void paint(Graphics g){
		...
		g.drawRect(25, 25, 400, 400);
		g.drawOval(25, 25, 400, 400);
		
		g.setColor(new Color(64, 128, 191));
		for(int i=0;i<nrAruncari;i++){
			g.setColor(new Color(127+(int)(Math.random()*127),127+(int)(Math.random()*127),127+(int)(Math.random()*127)));
