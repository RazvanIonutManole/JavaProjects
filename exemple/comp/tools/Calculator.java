...
public Calculator(Exemple ex){
       this.ex = ex;
       setTitle("Calculator");
       setBackground(new Color(38, 104, 165));
       setForeground(new Color(255,255,255));
       setResizable(false);
       setIconImage(ex.calculator);
       resize(350,400);
       setLayout (null);
       add(tf);
	...
	if ((e.target).equals(b[14])) {
		switch(op){
			...
			case 17:
				...
				break;		
			case 24:
				n =Math.pow(n, Double.parseDouble(tf.getText()));
				break;
			
		}
		String ss = "" + n;
		if(ss.endsWith(".0")) ss = ss.substring(0, ss.length()-2);
		tf.setText(ss); 
		return true;
	} 
	if ((e.target).equals(b[15])) {tf.setText("0");  flag = false; return true;} 
